...
function formatValidUntilShort(value: string): string {
  const parsed = new Date(value);
  if (Number.isNaN(parsed.getTime())) return value;
  return parsed.toLocaleDateString("fr-FR");
}
...
/**
 * Application de la fenêtre « access » : verrou local + activation de licence.
 *
 * N’importe JAMAIS `AppDataProvider` ni `getDatabase`.
 */
export function AccessApp() {
  ...
  async function handleActivated(newValidUntil: string) {
    setActivationSuccess(
      `Licence activée. Le droit d’utilisation est prolongé jusqu’au ${formatValidUntilShort(
        newValidUntil,
      )}. Saisissez maintenant votre mot de passe.`,
    );
    await refresh();
  }
  ...
  if (screen === "clockAnomaly" || screen === "expired") {
    return (
  ...
  return (
    <UnlockScreen
      status={status}
      successMessage={activationSuccess}
      onCompleted={refresh}
    />
  );
}
...
function LicenseBlockedScreen({
  ...
  mode: "expired" | "clockAnomaly";
  status: LocalAccessStatusDto | null;
  onActivated: (newValidUntil: string) => void | Promise<void>;
}) {
  ...
  async function handleActivate(event: FormEvent<HTMLFormElement>) {
    ...
    await onActivated(outcome.activation.newValidUntil);
  }

  return (
    <div className="boot-screen" role="alert">
      <div className="boot-screen__card boot-screen__card--error">
        <p className="boot-screen__eyebrow">
          {mode === "clockAnomaly" ? "Anomalie détectée" : "Période initiale terminée"}
        </p>
        <h1>
          {mode === "clockAnomaly" ? "Horloge système suspecte" : "Droit d’utilisation expiré"}
        </h1>
        {mode === "clockAnomaly" ? (
          <p>
            La date système semble avoir été modifiée. Vérifiez l’horloge ou contactez JRB
            XSolutions. Vous pouvez aussi activer un code de licence valide pour régulariser
            l’installation.
          </p>
        ) : (
          <p>
            Demandez un code de licence à JRB XSolutions pour prolonger l’utilisation de cette
            installation.
          </p>
        )}
        ...
        <form className="form-grid" onSubmit={handleActivate} noValidate>
          ...
          <div className="form-actions field--full">
            <button type="submit" className="button button--primary" disabled={submitting}>
              {submitting ? "Activation…" : "Activer la licence"}
            </button>
            <button
              type="button"
